...
def edit():
    ...
    conn = sqlite3.connect('C:/Users/STUDENTS/sammaamsol.db')
    c = conn.cursor()
    ...
    c.execute("SELECT * FROM sammy WHERE oid=?", (record_id,))
    record = c.fetchone()
...
def submit():
    conn = sqlite3.connect('C:/Users/STUDENTS/sammy.db')
    c = conn.cursor()

    c.execute("INSERT INTO sammy VALUES (:f_name, :l_name, :age, :address, :email)",
              {
                'f_name': f_name.get(),
                'l_name': l_name.get(),
                'age': age.get(),
...
def query():
    conn = sqlite3.connect('C:/Users/STUDENTS/sammy.db')
    c = conn.cursor()
    c.execute("SELECT *, oid FROM sammy")
    records = c.fetchall()
    conn.close()
...
def delete():
    conn = sqlite3.connect('C:/Users/STUDENTS/sammy.db')
    c = conn.cursor()
    c.execute("DELETE FROM sammy WHERE oid=?", (delete_box.get(),))
    conn.commit()
